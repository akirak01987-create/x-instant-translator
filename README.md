...
- **通知履歴**: 受信した投稿を新しい順に一覧表示（受信日時・投稿者・原文・日本語訳）。項目タップで元のX投稿（アプリ稼働中はその投稿、再起動後などは代わりにXアプリ）を開きます。個別削除・全件削除に対応し、端末再起動後も履歴は保持されます。保存件数の上限は「アプリ設定」画面から変更可能（初期値500件）。
...
- **アプリ設定**: 通知アクセスの許可、翻訳モデルの準備、保存件数上限の変更などを行います。
